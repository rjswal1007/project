...
const Auth = () => {
  ...
  const registerSubmitHandler = async (e) => {
    ...
    if (password !== confirmPassword) {
      ...
      try {
        ...
        setLoading(true);
        const { data } = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/users/auth/register`, {
  ...
  const loginSubmitHandler = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      const { data } = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/users/auth/login`, {
        email, password
      }, config);
      localStorage.setItem('userInfo', JSON.stringify(data));
      setLoading(false);
